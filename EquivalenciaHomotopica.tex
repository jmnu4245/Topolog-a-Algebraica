...
\begin{teorema}{Teorema de Poincaré}{teo_poincare}
Sea $X$ e.t. y $x_0 \in X$, entonces $(\pi_1(X, x_0), \cdot)$ es un grupo.
\end{teorema}
\begin{proof}
1. El producto "$\cdot$" está bien definido: Si $[\alpha] = [\alpha']$ y $[\beta] = [\beta'] \implies [\alpha * \beta] = [\alpha' * \beta']$.
Sabiendo que $[\alpha] = [\alpha'] \implies \exists H^{(1)} \colon I \times I \to X$ continua tal que: \\
$H^{(1)}_0 = \alpha, \quad H^{(1)}_1 = \alpha'$ \\
$H^{(1)}(0,t) = \alpha(0) = \alpha'(0) = x_0 \quad \forall t \in [0,1]$ \\
$H^{(1)}(1,t) = \alpha(1) = \alpha'(1) = x_0 \quad \forall t \in [0,1]$.

(El resto de la prueba sigue verificando la existencia de elemento neutro $[c_{x_0}]$, elemento inverso $[\alpha^{-1}]$ y la asociatividad módulo homotopía).
\end{proof}